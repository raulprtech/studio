...
function ImageUploadDialog({ open, onOpenChange, onImageUploaded }: { open: boolean, onOpenChange: (open: boolean) => void, onImageUploaded: (url: string) => void}) {
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        startUploading(async () => {
            const result = await uploadFileAction(formData, 'projects'); // Upload to 'projects' folder
            if (result.success && result.url) {
                onImageUploaded(result.url);
                toast({ title: "Imagen Subida", description: "La imagen se ha insertado en tu proyecto." });
                onOpenChange(false);
            } else {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>Sube una imagen para insertarla en la descripción de tu proyecto.</DialogDescription>
                </DialogHeader>
                <div className="py-4">
                        ...
                        <CardContent className="p-0">
                            ...
                            {viewMode === 'edit' ? (
                                <Textarea 
                                    ...
                                    ref={textAreaRef}
                                    placeholder="Escribe la descripción de tu proyecto aquí..." 
                                    value={content}
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <input type="hidden" name="coverImageUrl" value={coverImageUrl || ""} />
                             ...
                             <div className="grid gap-2">
                                ...
                                {coverImageUrl && <Image src={coverImageUrl} alt="Preview" width={1200} height={630} className="rounded-md aspect-video object-cover" data-ai-hint="project cover" />}
